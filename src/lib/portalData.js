...
export async function loadStudentAcademicData(studentId, enrollmentNumber, branch, semester) {
  ...
  const [attendance, results, assignments, studentAssignments, approvedSolutions, feeStatus, feeReceipts, octopodFees] = await Promise.all([
  ...
  return { attendance, results, assignments, studentAssignments, approvedSolutions, feeStatus: feeStatus[0] || octopodFees.data?.feeStatus, feeReceipts: feeReceipts.length ? feeReceipts : (octopodFees.data?.feeReceipts || []), octopodProfile: octopodFees.data?.profile || null };
}
